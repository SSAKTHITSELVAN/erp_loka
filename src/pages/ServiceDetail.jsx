...
export default function ServiceDetail() {
  ...
  const service = servicesData.modules.find(s => s.slug === slug);
    ...
    <main className="pt-16" style={{ background: '#0A0A0A' }}>
      ...
      <section className="relative min-h-[50vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <img src={service.image} alt={service.title} className="w-full h-full object-cover" style={{ filter: 'brightness(0.55)' }} />
      ...
      <section className="py-16" style={{ background: '#111111' }}>
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            Key <span style={{ color: '#D9B24C' }}>Services</span>
          </motion.h2>
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-4" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
              Need <span style={{ color: '#D9B24C' }}>{service.shortTitle}</span> Support?
            </h2>
            <p className="text-lg mb-8" style={{ color: '#CFCFCF' }}>
              Schedule a consultation with our SAP experts today.
            </p>
            <button
